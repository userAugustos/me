...
const PREVIEW_PATTERN =
  /<!--\s*preview:start\s*-->([\s\S]*?)<!--\s*preview:end\s*-->/i;
const KINDS = new Set(['essay', 'repo', 'post', 'note']);
...
function readKind(file, frontmatter) {
  ...
  if (!KINDS.has(kind))
    fail(file, `frontmatter 'kind' must be one of ${[...KINDS].join(', ')}`);
  return kind;
}
...
async function readPost(file) {
  ...
  if (!preview)
    fail(file, 'missing <!-- preview:start --> ... <!-- preview:end --> block');
  ...
  const body = removeLeadingTitle(
    parsed.content.replace(PREVIEW_PATTERN, ''),
    title,
  );
...
async function main() {
  ...
  await Promise.all(
    posts.map(post => {
      ...
      return fs.writeFile(
        path.join(GENERATED_DIR, `${post.slug}.json`),
