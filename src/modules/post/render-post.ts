...
export function renderGeneratedPost(root: HTMLElement, post: GeneratedPost): void {
  ...
  root.innerHTML = `
    <article class="pt-10 max-w-190">
      <a href="/" data-home-link class="inline-flex items-center gap-2 font-mono text-[11px] uppercase tracking-[0.1em] text-ink-3 no-underline transition-colors duration-300 hover:text-accent mb-9">← Field notes</a>
...
export function renderPostError(root: HTMLElement, retry: () => void): void {
  root.innerHTML = `
    <div class="py-16 max-w-160 font-mono text-[12px] text-ink-3">
      ...
      <a href="/" data-home-link class="ml-4 underline text-ink hover:text-accent">Back home</a>
    </div>
  `
