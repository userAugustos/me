...
function pick<T extends HTMLElement>(root: ParentNode, selector: string): T {
  ...
  if (!element)
    throw new Error(`topbar: missing element matching '${selector}'`);
  return element;
}
...
function buildRoot(): HTMLElement {
  ...
  if (!(root instanceof HTMLElement))
    throw new Error('topbar: template has no root element');
  return root;
}
...
function syncLanguageToggle(root: HTMLElement): void {
  ...
  root
    .querySelectorAll<HTMLButtonElement>('button[data-locale-option]')
    .forEach(button => {
      button.setAttribute(
        'aria-pressed',
        button.dataset.localeOption === locale ? 'true' : 'false',
      );
    });
}
